...
## Is my data safe?
Yes, your data is safe. We does not store your documents or
questions. All uploaded data is deleted after you close the browser tab.

## Why does it take so long to index my document?
We are using a free OpenAI API key, it will take a while to index
...
No, the answers are not 100% accurate. It uses GPT-3 to generate
answers. GPT-3 is a powerful language model, but it sometimes makes mistakes 
and is prone to hallucinations. Also, We uses semantic search
...
Always check with the sources to make sure that the answers
are correct.
"""
    )
